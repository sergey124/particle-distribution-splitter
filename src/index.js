...
$(function () {
  ...
  xMax = series[series.length - 1][0];

  seriesStorage.push(series);
  seriesStarts.push(0);
  seriesMedians.push(series[Math.trunc(series.length / 2)][0]);

  document.getElementById('splitButton').addEventListener("click", splitAndRedraw);

  let plot = $.plot("#placeholder", [series]);
  xMax = plot.getAxes().xaxis.from;
  yMax = plot.getAxes().yaxis.max;
});

function splitAndRedraw() {
  splitSeries();

  let placeholder = $("#placeholder");

  let plot = $.plot(placeholder, seriesStorage);

  let leftBottom = plot.pointOffset({ x: 0, y: 0 });
  let rightTop = plot.pointOffset({ x: xMax, y: yMax });

  // Append max marks
  let circlePoints = [];
  for (let i = 0; i < seriesStorage.length; i++) {
    let series = seriesStorage[i];
    let median = seriesMedians[i];
    let values = series.map(point => point[0]);
    let medianIndex = binarySearch(values, median, compare_number);
  ...
  let seriesStorageWithBeauty = [...seriesStorage]
...
function splitSeries() {
  ...
  if (isNaN(splitPoint) || splitPoint < 0 || splitPoint >= xMax) {
  ...
  let seriesIndex = getInsertionPoint(binarySearch(seriesStarts, splitPoint, compare_number));

  if (splitPoint === seriesStarts[seriesIndex]) {
    return;
  ...
  // split this series
  let splitted = splitSeriesInternal(seriesStorage[seriesIndex], splitPoint, splitRange);
  seriesStorage[seriesIndex] = splitted[0];

  let nextIndex = seriesIndex + 1;
  seriesStorage.splice(nextIndex, 0, splitted[1]);
  seriesStarts.splice(nextIndex, 0, splitPoint);

  let median1 = probabilityMedian(seriesStorage[seriesIndex]);
  let median2 = probabilityMedian(seriesStorage[nextIndex]);

  seriesMedians[seriesIndex] = median1;
  seriesMedians.splice(nextIndex, 0, median2);
}

function probabilityMedian(probabilities) {
  let sum = 0;
...
function splitSeriesInternal(series, splitPoint, splitRange) {
  let series1 = [];
  let i = 0;
  ...
  let endSplitIndex = indexOfValueHigherThan(series, splitPoint + splitRange);
...
function indexOfValueHigherThan(array, value) {
...
function getInsertionPoint(binarySearchResult) {
